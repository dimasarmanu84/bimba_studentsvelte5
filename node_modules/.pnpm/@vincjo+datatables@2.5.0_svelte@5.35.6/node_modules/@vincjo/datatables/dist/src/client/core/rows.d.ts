...
export declare const data: {
    ...
    filter: <Row>(allRows: $state.Snapshot<Row[]>, { callback, value, check, key }: Filter<Row>, highlight?: boolean) => $state.Snapshot<Row>[];
    query: <Row>(allRows: $state.Snapshot<Row[]>, { path, key, value, check }: Query<Row>) => $state.Snapshot<Row>[];
};
